        ...
        <table className="w-full text-left text-gray-300">
          <thead>
            <tr className="border-b border-gray-600">
              <th className="px-4 py-2 whitespace-pre">Date and Time</th>
              <th className="px-4 py-2">Coin</th>
              <th className="px-4 py-2">Account</th>
              <th className="px-4 py-2">Amount</th>
              <th className="px-4 py-2 whitespace-pre">Bonus Earned</th>
            </tr>
          </thead>
          <tbody>
            ...
            {historyTable && historyTable?.map((item:any,index:number)=>{
              ...
              <td className="px-4 py-2">{item?.coin==="Native" ? "BNB" :item?.coin || "--"}</td>
              <td className="px-4 py-2">{item?.user || "--"}</td>
              <td className="px-4 py-2">
              <p >
                {` ${item?.volume ? Number(formatEther(BigInt(item?.volume))).toFixed(4) :"--"} DCRX`|| "--"}
                </p>
